...
def example_script(name_interface):
    ...
    while ((not robot_if.IsTimeout())
        ...
        if ((clock() - last) > dt):
            ...
            if (clock() < 10): # stop sending commands after 10s, to show that the sensors feedback still works after the master bord times out after not receiving any commands (WIFI_ETH_ERROR state)
