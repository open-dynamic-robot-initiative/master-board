...
class BenchmarkData(object):
    ...
    def dump_data():
        ...
        # creation of the folder where the graphs and data will be stored
        data_folder = Path("/tmp") / "communication_benchmark"
        if not data_folder.isdir():
        ...
        plt.savefig(data_folder + dir_name + '/' + dir_name + "-wifi-eth-latency.png")
        ...
        plt.savefig(data_folder + dir_name + '/' + dir_name + "-wifieth-stats.png")
        ...
        plt.savefig(data_folder + dir_name + '/' + dir_name + "-command-loop-duration.png")


        # creation of the text file with system info
        text_file = open(data_folder + dir_name + '/' + dir_name + "-system-info.txt", 'w')
...
def communication_benchmark(name_interface):
    N_UDRIVER_CONTROLED = 1  # Current number of controled drivers
    ...
    for i in range(N_UDRIVER_CONTROLED):  #  We enable each controler driver and its two associated motors
    ...
    if robot_if.IsTimeout():
        ...
        for i in range(N_UDRIVER_CONTROLED):
